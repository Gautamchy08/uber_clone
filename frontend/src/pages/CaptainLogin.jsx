...
const CaptainLogin = () => {
  ...
  const submitHandler = async e => {
    ...
    try {
      ...
      if (error.response) {
        console.log('error occurred here', error.response.data.message)
      }
    }
  }
  return (
    <div className='p-7 flex flex-col justify-between h-screen'>
      <div className='p-7'>
        ...
        >
          ...
          <input
            className='bg-[#EEEEEE] rounded px-2 py-4 mb-7 border w-full text-lg placeholder:text-base'
            required
            value={email}
          ...
          />
          <button className='bg-[#111]  font-semibold text-white rounded px-2 py-4 mb-7 border w-full text-lg  '>
            Login
          </button>
      ...
      <div>
        <Link
          to='/login'
          className='bg-[#272B3A] flex items-center justify-center font-semibold text-white rounded px-2 py-4 mb-7 border w-full text-lg  '
        >
          Sign in as User
        </Link>
      </div>
